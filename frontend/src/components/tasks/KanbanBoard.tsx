      ...
      {columns.map((column) => (
        ...
        >
          <div className="mb-4 flex items-center justify-between">
            <h3 className="panel-title text-xl font-semibold text-foreground">{column.label}</h3>
            <span className="rounded-full border border-border/70 bg-card/70 px-3 py-1 text-xs font-semibold text-muted-foreground">
              ...
              return (
                <article
                  ...
                  className={clsx(
                    ...
                    dueSoon && !overdue && "border-warning/40 bg-warning/10",
                ...
                >
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {task.assignees.map((assignee) => (
                      <span key={assignee.id} className="rounded-full border border-border/70 bg-card/70 px-3 py-1 text-xs font-medium text-foreground">
                        {assignee.name}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="mt-4 border-t border-border/70 pt-4">
                    <button
                      ...
                      className="ghost-button rounded-full px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-foreground"
                    >
                      {isExpanded
                        ? "Hide comments & suggestions"
                        : `Comments & Suggestions${typeof task.commentsCount === "number" ? ` (${task.commentsCount})` : ""}`}
                    </button>
                      ...
                      <div className="mt-4 space-y-3" onMouseDown={(event) => event.stopPropagation()}>
                        <div className="space-y-2">
                          <div className="flex items-center justify-between gap-3">
                            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">Assigned teammates</p>
                            {updatingAssigneesTaskId === task.id ? (
                              <span className="text-[11px] font-medium text-accent">Updating...</span>
                            ) : null}
                          </div>
                                ...
                                <button
                                  ...
                                  className={clsx(
                                    ...
                                    checked
                                      ...
                                      : "ghost-button text-muted-foreground hover:text-foreground"
                                  )}
                                >
                        ...
                        <div className="max-h-48 space-y-3 overflow-y-auto pr-1">
                          {loadingCommentsTaskId === task.id ? (
                            ...
                            comments.map((comment) => (
                              <div key={comment.id} className="rounded-[20px] border border-border/70 bg-card/65 px-3 py-3">
                                <div className="flex items-center justify-between gap-3">
                                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-accent">{comment.user.name}</p>
                            ...
                            <p className="text-sm text-muted-foreground">No comments yet on this task.</p>
                          )}
                        </div>
                        ...
                        {isAssignedToTask ? (
                          ...
                          <p className="text-sm text-muted-foreground">Only teammates assigned to this task can add comments.</p>
                        )}
                      </div>
                    ) : null}
                  </div>
